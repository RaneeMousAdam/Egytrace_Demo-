...
function renderRegulatory() {
  ...
  function refCard(r) {
    ...
    <div class="reg-card">
      ...
      ${r['Page / section pointer'] ? `<div style="font-size:11px;color:var(--text-muted);margin-bottom:8px;display:flex;align-items:center;gap:4px;">${renderIcon('tag', 12)} ${escHtml(r['Page / section pointer'])}</div>` : ''}
      ${r.Notes ? `<div style="font-size:11.5px;color:var(--text-secondary);margin-bottom:10px;line-height:1.5;">${escHtml(r.Notes)}</div>` : ''}
      ${r['Official source URL'] ? `<a href="${escHtml(r['Official source URL'])}" target="_blank" rel="noopener" class="reg-url-btn">${renderIcon('external-link', 12)} Official Source ↗</a>` : ''}
    </div>`;
  }
  ...
  <div class="page-header">
    <div class="page-title-row">
      <div class="page-title-wrap">
        ${renderIcon('book-open', 20, 'page-title-icon')}
        <h2>Regulatory References</h2>
      </div>
      <div class="page-header-actions">
        <span class="badge badge-info">${official.length} Official EU</span>
        <span class="badge badge-accent">${external.length} External Standards</span>
      </div>
    </div>
    <div class="page-desc">EU CBAM Implementing Regulations, EU ETS Monitoring & Reporting Regulation (MRR), and GCCA/ISO standards.</div>
  </div>

  ${buildDemoBanner(s)}

  <div style="font-size:12px;color:var(--text-muted);margin-bottom:var(--sp-lg);padding:var(--sp-sm) var(--sp-md);background:var(--bg-card);border-radius:var(--r-sm);border:1px solid var(--border);display:flex;align-items:center;gap:8px;">
    ${renderIcon('info', 14)}
  ...
  <div style="margin-bottom:var(--sp-md);">
    <div style="font-size:11px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.09em;margin-bottom:var(--sp-md);">Official EU / CBAM References</div>
